...
// =============================================================================
// 🔧 SCRIPT TEMPORÁRIO: MIGRAÇÃO DOS PALPITES DO GRUPO B
// =============================================================================

const btnMigrar = document.getElementById('btn-migrar-grupob');

if (btnMigrar) {
  btnMigrar.addEventListener('click', async () => {
    const confirmar = confirm("🚨 ATENÇÃO ADMIN: Isso vai ler o banco de dados e mover os palpites B3/B4 para a Rodada 3, e B5/B6 para a Rodada 2 de TODOS os jogadores automaticamente.\n\nTem certeza que deseja executar?");
    
    if (!confirmar) return;

    btnMigrar.innerText = "⏳ Processando migração...";
    btnMigrar.style.backgroundColor = "#7f8c8d";

    try {
      const palpitesSnap = await getDocs(collection(db, "palpites"));
      let jogadoresCorrigidos = 0;

      for (const userDoc of palpitesSnap.docs) {
        const dados = userDoc.data();
        let precisaAtualizar = false;
        
        const rodada2 = dados.rodada2 || {};
        const rodada3 = dados.rodada3 || {};

        // Faz o backup dos palpites antigos nas posições erradas
        const palpiteOldB3 = rodada2["B3"]; // Era Canadá x Suíça
        const palpiteOldB4 = rodada2["B4"]; // Era Bósnia x Catar
        const palpiteOldB5 = rodada3["B5"]; // Era Canadá x Catar
        const palpiteOldB6 = rodada3["B6"]; // Era Bósnia x Suíça

        // Se o cara palpitou em qualquer um desses 4 jogos, o robô age:
        if (palpiteOldB3 || palpiteOldB4 || palpiteOldB5 || palpiteOldB6) {
            
            // 1. O que era Rodada 2 vai para a Rodada 3
            if (palpiteOldB3) rodada3["B5"] = { timeA: palpiteOldB3.timeA, timeB: palpiteOldB3.timeB };
            if (palpiteOldB4) rodada3["B6"] = { timeA: palpiteOldB4.timeA, timeB: palpiteOldB4.timeB };
            
            // 2. O que era Rodada 3 (B5) vem para a Rodada 2 (B3) direto
            if (palpiteOldB5) rodada2["B3"] = { timeA: palpiteOldB5.timeA, timeB: palpiteOldB5.timeB };

            // 3. 🚨 A MÁGICA: O que era Rodada 3 (B6) vem para Rodada 2 (B4) INVERTENDO OS GOLS
            // Antigo: timeA (Bósnia), timeB (Suíça). Novo: timeA (Suíça), timeB (Bósnia).
            if (palpiteOldB6) {
                rodada2["B4"] = { 
                    timeA: palpiteOldB6.timeB, // Gols da Suíça vão para o lado A
                    timeB: palpiteOldB6.timeA  // Gols da Bósnia vão para o lado B
                };
            }

            // Envia o documento reescrito de volta para o Firebase
            await setDoc(doc(db, "palpites", userDoc.id), {
              rodada2: rodada2,
              rodada3: rodada3
            }, { merge: true });
        ...
        if (precisaAtualizar) jogadoresCorrigidos++;
      }

      alert(`✅ MÁGICA CONCLUÍDA!\nOs palpites de ${jogadoresCorrigidos} jogadores foram realocados com sucesso.`);
      window.location.reload(); // Recarrega para você ver os números nos lugares certos

    } catch (error) {
      console.error("Erro na migração:", error);
      alert("Falha ao tentar executar o script de migração.");
      btnMigrar.innerText = "🔧 CORRIGIR PALPITES DO GRUPO B NO BANCO";
      btnMigrar.style.backgroundColor = "#d35400";
    }
  });
}
